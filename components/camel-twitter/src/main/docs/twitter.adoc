...
The Twitter component supports 44 endpoint options which are listed below:
...
| truncate | common | false | boolean | Set the request for truncation: if true tweets longer than 140 chars will be truncated; otherwise those updates may result in an error (e.g. TwitterException).
